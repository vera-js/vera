...
test('multiple text parts in one element', () => {
  renderInto(html`<p>${'a'}-${'b'}</p>`, el);
  assert.equal(read(), '<p>a-b</p>');
  renderInto(html`<p>${'c'}-${'d'}</p>`, el);
...
test('boolean attribute toggles', () => {
  renderInto(html`<input ?disabled=${true} />`, el);
  assert.equal(el.querySelector('input').hasAttribute('disabled'), true);
  renderInto(html`<input ?disabled=${false} />`, el);
...
test('element-position: refs fire, junk is ignored, alignment holds', () => {
  ...
  renderInto(html`<div ${(n) => (seen = n)} class="${'c'}"><input ${box} />${'t'}</div>`, el);
  ...
  renderInto(html`<div ${'junk'} class="${'c2'}">${'t2'}</div>`, el); // non-ref value: ignored, no throw
...
test('two containers are independent', () => {
  ...
  renderInto(html`<p>${'one'}</p>`, el);
  renderInto(html`<p>${'two'}</p>`, el2);
...
test('unkeyed list grows and shrinks', () => {
  ...
  renderInto(ul([1, 2].map(li)), el);
  assert.deepEqual(texts(), ['1', '2']);
  renderInto(ul([1, 2, 3, 4].map(li)), el);
  assert.deepEqual(texts(), ['1', '2', '3', '4']);
  renderInto(ul([9].map(li)), el);
...
test('keyed swap moves nodes instead of rewriting them', () => {
  ...
  const [a, b, c, d] = items();
  renderInto(ul(data(1, 3, 2, 4).map(row)), el);
...
test('keyed reverse preserves node identity', () => {
  ...
  const before = items();
  renderInto(ul(data(5, 4, 3, 2, 1).map(row)), el);
...
test('keyed update in place', () => {
  ...
  const before = items();
  renderInto(ul([{ id: 1, label: 'L1' }, { id: 2, label: 'CHANGED' }, { id: 3, label: 'L3' }].map(row)), el);
...
test('list switching to a template and back', () => {
  ...
  renderInto(t(html`<b>solo</b>`), el);
  assert.equal(read(), '<div><b>solo</b></div>');
  renderInto(t(data(3).map(row)), el);
...
test('keyed item that changes template shape under the same key (element-mode demote)', () => {
  ...
  renderInto(ul(data(1, 2, 3).map(rowA)), el);
  assert.deepEqual(texts(), ['A:L1', 'A:L2', 'A:L3']);
  // same keys, two items switch to a different template shape
  renderInto(ul([rowB({ id: 1, label: 'L1' }), rowA({ id: 2, label: 'L2' }), rowB({ id: 3, label: 'L3' })]), el);
  ...
  renderInto(ul([rowA({ id: 3, label: 'L3' }), rowA({ id: 1, label: 'L1' })]), el);
...
test('multi-root keyed items reconcile and move correctly (markered path)', () => {
  ...
  renderInto(ul(data(1, 2, 3).map(pair)), el);
  assert.deepEqual(texts(), ['1a', '1b', '2a', '2b', '3a', '3b']);
  renderInto(ul(data(3, 1).map(pair)), el);
  assert.deepEqual(texts(), ['3a', '3b', '1a', '1b']);
  renderInto(ul(data(2, 3, 1).map(pair)), el);
...
test('mixed single-root and multi-root keyed items', () => {
  ...
  renderInto(ul([one({ id: 1 }), two({ id: 2 }), one({ id: 3 })]), el);
  assert.deepEqual(texts(), ['1solo', '2a', '2b', '3solo']);
  renderInto(ul([one({ id: 3 }), two({ id: 2 }), one({ id: 1 })]), el);
...
test('hold() preserves DOM and element state across a toggle', () => {
  ...
  renderInto(t(false, 2), el); // and the held branch still takes fresh values
...
test('a text value containing markup renders as text, not elements', () => {
  renderInto(html`<div>${'<img src=x onerror=alert(1)>'}</div>`, el);
...
test('a value cannot break out of an attribute', () => {
  renderInto(html`<div title="${'" onmouseover="steal()'}">x</div>`, el);
...
test('a value cannot introduce a script element', () => {
  renderInto(html`<div>${'</div><script>alert(1)</script>'}</div>`, el);
...
test('markup in a keyed list item is text too', () => {
  ...
  renderInto(rows(['<b>a</b>', '<i>b</i>']), el);
...
test('a DOM node renders as itself', () => {
  ...
  renderInto(html`<div>${span}</div>`, el);
...
test('a document fragment renders its children', () => {
  ...
  renderInto(html`<div>${template.content.cloneNode(true)}</div>`, el);
...
test('one node replaces another', () => {
  ...
  renderInto(html`<div>${a}</div>`, el);
  renderInto(html`<div>${b}</div>`, el);
...
test('nodes render inside arrays and keyed lists', () => {
  ...
  renderInto(html`<ul>${nodes}</ul>`, el);
  assert.equal(read(), '<ul><li>1</li><li>2</li><li>3</li></ul>');
  renderInto(html`<ul>${nodes.map((node, i) => keyed(i, node))}</ul>`, el);
...
test('a live property reasserts after a sibling changed the DOM', () => {
  ...
  document.body.appendChild(el);
  const draw = (picked) => renderInto(html`
...
test('a plain property still leaves what a person typed', () => {
  const el = document.createElement('div');
  document.body.appendChild(el);
  const draw = (v) => renderInto(html`<input .value=${v} />`, el);
  draw('Ada');
  const input = el.querySelector('input');
  input.value = 'Grace';
  draw('Ada');
