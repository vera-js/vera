...
| `when` | host-native condition — **selector in vera, expression in omni** (see DIVERGENCES) | **gates**: animates while true, rests at its start otherwise; `when` + `play` = a UI transition |
| `run-once` | boolean | first forward play latches at the end, forever — survives gates closing and attribute edits |
| `anchor` | `'self'` or a selector | whose box `scroll` measures |
| `ease` | CSS timing function, verbatim | per-SEGMENT curve (the CSS model on both sides); refused with `play` only until vera's write-path flip, then composes |
