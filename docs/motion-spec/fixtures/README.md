...
| hash vectors, BOTH widths (published references + generated-CSS rule bodies; `fnv1a64` omni, `fnv1a32` vera — width divergence recorded in DIVERGENCES) | this directory: `hash-vectors.json` (omni-contributed 2026-09-09 as `fnv1a64-vectors.json`; vera added the 32-bit column on adoption, each value verified against shipping `contentHash`) | omni: BigInt + PHP 16-bit-limb pair; vera: verified on adoption, suite pin pending |
