...
**A working VeraJS app is about <!--size:app.kb-->5.5 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->5 596 B<!--/size:app.bytes-->) — <!--size:app.rank-->4th<!--/size:app.rank--> of
...
| **VeraJS + lit-html** | **5 196 B** | 4.3x |
| **VeraJS + own renderer** | **5 596 B** | 4.6x |
| Lit | 5 871 B | 4.8x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 230 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->5 596 B<!--/size:app.bytes-->, because a bundler
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->6 047 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->779 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
anyone can ship. Quoting core's standalone <!--size:core.gzip-->2.62 KB<!--/size:core.gzip--> as an
...
VeraJS + own renderer is <!--size:app.bytes-->5 596 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->5 596 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
pairing (<!--size:app.verajs-lit-html.bytes-->5 196 B<!--/size:app.verajs-lit-html.bytes-->) is lower still. A gap that
...
| Module | gzip | |
| --- | ---: | --- |
| `@verajs/core` | 2 679 B | state (incl. Map and Set), hooks, lifecycle, render |
