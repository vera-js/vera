...
**A working VeraJS app is about <!--size:app.kb-->6.0 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->6 123 B<!--/size:app.bytes-->) — <!--size:app.rank-->6th<!--/size:app.rank--> of
...
| **VeraJS + lit-html** | **5 438 B** | 4.5x |
| Lit | 5 871 B | 4.8x |
| Preact + signals | 6 031 B | 4.9x |
| **VeraJS + own renderer** | **6 123 B** | 5.0x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 872 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->6 123 B<!--/size:app.bytes-->, because a bundler
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->6 582 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->244 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
(<!--size:app.static.bytes-->5 054 B<!--/size:app.static.bytes-->) —
<!--size:app.static.underlit-->817 B<!--/size:app.static.underlit--> under Lit.**
...
anyone can ship. Quoting core's standalone <!--size:core.gzip-->2.97 KB<!--/size:core.gzip--> as an
...
VeraJS + own renderer is <!--size:app.bytes-->6 123 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->6 123 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
pairing (<!--size:app.verajs-lit-html.bytes-->5 438 B<!--/size:app.verajs-lit-html.bytes-->) is lower still. A gap that
...
| Module | gzip | |
| --- | ---: | --- |
| `@verajs/core` | 3 046 B | state (incl. Map and Set), hooks, lifecycle, render |
| `@verajs/renderer` | 3 826 B | keyed template renderer, refs, `hold` |
| `@verajs/router` | 3 672 B | nested routes, params, wildcards, redirects, scroll memory |
