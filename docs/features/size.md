...
**A working VeraJS app is about <!--size:app.kb-->5.7 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->5 882 B<!--/size:app.bytes-->) — <!--size:app.rank-->5th<!--/size:app.rank--> of
...
| Lit | 5 871 B | 4.8x |
| **VeraJS + own renderer** | **5 882 B** | 4.8x |
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->6 338 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->488 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
VeraJS + own renderer is <!--size:app.bytes-->5 882 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->5 882 B<!--/size:app.bytes-->) is within ten bytes of Lit
