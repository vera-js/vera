...
**A working VeraJS app is about <!--size:app.kb-->6.9 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->7 025 B<!--/size:app.bytes-->) — <!--size:app.rank-->6th<!--/size:app.rank--> of
...
| **VeraJS + own renderer** | **7 025 B** | 5.8x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->7 904 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->7 025 B<!--/size:app.bytes-->, because a bundler
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->7 767 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->-941 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
(<!--size:app.static.bytes-->7 034 B<!--/size:app.static.bytes-->) —
<!--size:app.static.underlit-->-1 163 B<!--/size:app.static.underlit--> under Lit.**
...
VeraJS + own renderer is <!--size:app.bytes-->7 025 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->7 025 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
| `@verajs/renderer` | 4 654 B | keyed template renderer, refs, `hold` |
...
| `@verajs/renderer/spread` | 1 575 B | `${spread(props)}` — runtime-named bindings |
| `@verajs/renderer/tag` | 2 193 B | `<${tag}>` — runtime tag names, in templates and JSX |
...
| `@verajs/renderer/hydrate` | 6 399 B | INSTEAD OF `@verajs/renderer` on an SSR page — the adopting renderer, not an addition beside it |
