...
**A working VeraJS app is about <!--size:app.kb-->5.9 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->6 024 B<!--/size:app.bytes-->) — <!--size:app.rank-->5th<!--/size:app.rank--> of
...
| Lit | 5 871 B | 4.8x |
| **VeraJS + own renderer** | **6 024 B** | 4.9x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 695 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->6 024 B<!--/size:app.bytes-->, because a bundler
...
VeraJS + own renderer is <!--size:app.bytes-->6 024 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->6 024 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
| `@verajs/renderer` | 3 819 B | keyed template renderer, refs, `hold` |
