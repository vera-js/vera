...
**A working VeraJS app is about <!--size:app.kb-->6.0 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->6 138 B<!--/size:app.bytes-->) — <!--size:app.rank-->6th<!--/size:app.rank--> of
...
| **VeraJS + lit-html** | **5 421 B** | 4.4x |
| Lit | 5 871 B | 4.8x |
| Preact + signals | 6 031 B | 4.9x |
| **VeraJS + own renderer** | **6 138 B** | 5.0x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 946 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->6 138 B<!--/size:app.bytes-->, because a bundler
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->6 649 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->177 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
(<!--size:app.static.bytes-->5 065 B<!--/size:app.static.bytes-->) —
<!--size:app.static.underlit-->806 B<!--/size:app.static.underlit--> under Lit.**
...
**There is no smaller honest number than <!--size:app.kb-->6.0 KB<!--/size:app.kb-->.** Core ships no
renderer — `render()` with none registered warns and paints nothing — so "core alone" is not a tier
anyone can ship. Quoting core's standalone <!--size:core.gzip-->2.99 KB<!--/size:core.gzip--> as an
...
VeraJS + own renderer is <!--size:app.bytes-->6 138 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->6 138 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
pairing (<!--size:app.verajs-lit-html.bytes-->5 421 B<!--/size:app.verajs-lit-html.bytes-->) is lower still. A gap that
...
(<!--size:styles.gzip-->674 B<!--/size:styles.gzip--> gzipped) back, so the win belongs to apps that do not.
...
| Module | gzip | |
| --- | ---: | --- |
| `@verajs/core` | 3 061 B | state (incl. Map and Set), hooks, lifecycle, render |
| `@verajs/renderer` | 3 885 B | keyed template renderer, refs, `hold` |
...
| `@verajs/styles` | 674 B | `static styles` adoption, shadow and light DOM |
...
| `@verajs/renderer/keyed` | 664 B | `keyed()` — keyed list reconciliation |
