...
**A working VeraJS app is about <!--size:app.kb-->6.5 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->6 632 B<!--/size:app.bytes-->) — <!--size:app.rank-->6th<!--/size:app.rank--> of
...
| **VeraJS + own renderer** | **6 632 B** | 5.4x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->7 488 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->6 632 B<!--/size:app.bytes-->, because a bundler
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->7 387 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->-561 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
**<!--size:app.static.kb-->5.5 KB<!--/size:app.static.kb--> gzipped
(<!--size:app.static.bytes-->5 592 B<!--/size:app.static.bytes-->) —
<!--size:app.static.underlit-->279 B<!--/size:app.static.underlit--> under Lit.**
...
**There is no smaller honest number than <!--size:app.kb-->6.5 KB<!--/size:app.kb-->.** Core ships no
...
VeraJS + own renderer is <!--size:app.bytes-->6 632 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->6 632 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
| `@verajs/renderer` | 4 419 B | keyed template renderer, refs, `hold` |
| `@verajs/router` | 4 065 B | nested routes, params, wildcards, redirects, scroll memory |
...
| `@verajs/renderer/slots` | 3 294 B | `<slot>` distribution in a LIGHT-DOM component, and `slotted()` |
