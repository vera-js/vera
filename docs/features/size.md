...
**A working VeraJS app is about <!--size:app.kb-->6.2 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->6 371 B<!--/size:app.bytes-->) — <!--size:app.rank-->6th<!--/size:app.rank--> of
...
| **VeraJS + own renderer** | **6 371 B** | 5.2x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->7 187 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->6 371 B<!--/size:app.bytes-->, because a bundler
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->6 886 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->-60 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
(<!--size:app.static.bytes-->5 301 B<!--/size:app.static.bytes-->) —
<!--size:app.static.underlit-->570 B<!--/size:app.static.underlit--> under Lit.**
...
VeraJS + own renderer is <!--size:app.bytes-->6 371 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->6 371 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
| `@verajs/renderer` | 4 118 B | keyed template renderer, refs, `hold` |
...
| `@verajs/renderer/slots` | 2 237 B | `<slot>` distribution in a LIGHT-DOM component, and `slotted()` |
