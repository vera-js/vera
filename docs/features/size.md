...
**A working VeraJS app is about <!--size:app.kb-->7.0 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->7 145 B<!--/size:app.bytes-->) — <!--size:app.rank-->6th<!--/size:app.rank--> of
...
| **VeraJS + own renderer** | **7 145 B** | 5.9x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->8 024 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->7 145 B<!--/size:app.bytes-->, because a bundler
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->7 879 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->-1 053 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
(<!--size:app.static.bytes-->7 151 B<!--/size:app.static.bytes-->) —
<!--size:app.static.underlit-->-1 280 B<!--/size:app.static.underlit--> under Lit.**
...
VeraJS + own renderer is <!--size:app.bytes-->7 145 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->7 145 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
| `@verajs/renderer` | 4 685 B | keyed template renderer, refs, `hold` |
...
| `@verajs/jsx` | 4 001 B | the JSX transform + Vite plugin — build-time, never shipped to a page |
| `@verajs/jsx/standalone` | 4 203 B | JSX in the browser with no build — the CodePen path |
