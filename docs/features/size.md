...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 344 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->5 711 B<!--/size:app.bytes-->, because a bundler
...
anyone can ship. Quoting core's standalone <!--size:core.gzip-->2.64 KB<!--/size:core.gzip--> as an
...
(<!--size:app.verajs-lit-html.bytes-->5 249 B<!--/size:app.verajs-lit-html.bytes-->) is lower still. It was *above* Lit until
...
| Module | gzip | |
| --- | ---: | --- |
| `@verajs/core` | 2 705 B | state (incl. Map and Set), hooks, lifecycle, render |
