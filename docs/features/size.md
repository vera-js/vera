...
**A working VeraJS app is about <!--size:app.kb-->5.7 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->5 846 B<!--/size:app.bytes-->) — <!--size:app.rank-->4th<!--/size:app.rank--> of
...
| **VeraJS + lit-html** | **5 353 B** | 4.4x |
| **VeraJS + own renderer** | **5 846 B** | 4.8x |
| Lit | 5 871 B | 4.8x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 437 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->5 846 B<!--/size:app.bytes-->, because a bundler
...
anyone can ship. Quoting core's standalone <!--size:core.gzip-->2.72 KB<!--/size:core.gzip--> as an
...
VeraJS + own renderer is <!--size:app.bytes-->5 846 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->5 846 B<!--/size:app.bytes-->) is below Lit (<!--size:app.lit.bytes-->5 871 B<!--/size:app.lit.bytes-->) and
Preact + signals (<!--size:app.preact-signals.bytes-->6 031 B<!--/size:app.preact-signals.bytes-->); the lit-html pairing
(<!--size:app.verajs-lit-html.bytes-->5 353 B<!--/size:app.verajs-lit-html.bytes-->) is lower still. It was *above* Lit until
...
| Module | gzip | |
| --- | ---: | --- |
| `@verajs/core` | 2 787 B | state (incl. Map and Set), hooks, lifecycle, render |
