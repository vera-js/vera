...
**A working VeraJS app is about <!--size:app.kb-->5.6 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->5 699 B<!--/size:app.bytes-->) — <!--size:app.rank-->4th<!--/size:app.rank--> of
...
| **VeraJS + lit-html** | **5 206 B** | 4.3x |
| **VeraJS + own renderer** | **5 699 B** | 4.7x |
| Lit | 5 871 B | 4.8x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 295 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->5 699 B<!--/size:app.bytes-->, because a bundler
...
anyone can ship. Quoting core's standalone <!--size:core.gzip-->2.57 KB<!--/size:core.gzip--> as an
...
VeraJS + own renderer is <!--size:app.bytes-->5 699 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->5 699 B<!--/size:app.bytes-->) is below Lit (<!--size:app.lit.bytes-->5 871 B<!--/size:app.lit.bytes-->) and
Preact + signals (<!--size:app.preact-signals.bytes-->6 031 B<!--/size:app.preact-signals.bytes-->); the lit-html pairing
(<!--size:app.verajs-lit-html.bytes-->5 206 B<!--/size:app.verajs-lit-html.bytes-->) is lower still. It was *above* Lit until
...
(<!--size:styles.gzip-->555 B<!--/size:styles.gzip--> gzipped) back, so the win belongs to apps that do not.
...
| Module | gzip | |
| --- | ---: | --- |
| `@verajs/core` | 2 633 B | state (incl. Map and Set), hooks, lifecycle, render |
| `@verajs/renderer` | 3 662 B | keyed template renderer, refs, `hold` |
| `@verajs/router` | 3 513 B | nested routes, params, wildcards, redirects, scroll memory |
| `@verajs/autoloader` | 1 007 B | lazy component discovery |
| `@verajs/styles` | 555 B | `static styles` adoption, shadow and light DOM |
