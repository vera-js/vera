...
**A working VeraJS app is about <!--size:app.kb-->5.5 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->5 683 B<!--/size:app.bytes-->) — <!--size:app.rank-->4th<!--/size:app.rank--> of
...
| **VeraJS + lit-html** | **5 191 B** | 4.3x |
| **VeraJS + own renderer** | **5 683 B** | 4.7x |
| Lit | 5 871 B | 4.8x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 282 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->5 683 B<!--/size:app.bytes-->, because a bundler
...
VeraJS + own renderer is <!--size:app.bytes-->5 683 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->5 683 B<!--/size:app.bytes-->) is below Lit (<!--size:app.lit.bytes-->5 871 B<!--/size:app.lit.bytes-->) and
Preact + signals (<!--size:app.preact-signals.bytes-->6 031 B<!--/size:app.preact-signals.bytes-->); the lit-html pairing
(<!--size:app.verajs-lit-html.bytes-->5 191 B<!--/size:app.verajs-lit-html.bytes-->) is lower still. It was *above* Lit until
...
| `@verajs/router` | 3 509 B | nested routes, params, wildcards, redirects, scroll memory |
| `@verajs/autoloader` | 1 130 B | lazy component discovery |
