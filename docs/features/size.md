...
**A working VeraJS app is about <!--size:app.kb-->5.8 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->5 989 B<!--/size:app.bytes-->) — <!--size:app.rank-->5th<!--/size:app.rank--> of
...
| Lit | 5 871 B | 4.8x |
| **VeraJS + own renderer** | **5 989 B** | 4.9x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 647 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->5 989 B<!--/size:app.bytes-->, because a bundler
...
| | gzipped |
| --- | ---: |
| VeraJS + `@verajs/renderer` + `/keyed` | **<!--size:list.bytes-->6 446 B<!--/size:list.bytes-->** |
| Lit + `directives/repeat` | <!--size:list.lit.bytes-->6 826 B<!--/size:list.lit.bytes--> |

<!--size:list.vs-lit.bytes-->380 B<!--/size:list.vs-lit.bytes--> in our favour, against Lit's lead on the counter. Both numbers are real and both are
...
VeraJS + own renderer is <!--size:app.bytes-->5 989 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->5 989 B<!--/size:app.bytes-->) is within ten bytes of Lit
