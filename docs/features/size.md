...
**A working VeraJS app is about <!--size:app.kb-->5.7 KB<!--/size:app.kb--> gzipped (<!--size:app.bytes-->5 881 B<!--/size:app.bytes-->) — <!--size:app.rank-->5th<!--/size:app.rank--> of
...
| Lit | 5 871 B | 4.8x |
| **VeraJS + own renderer** | **5 881 B** | 4.8x |
...
`vera-renderer.min.js` is <!--size:stack.bytes-->6 482 B<!--/size:stack.bytes--> gzipped against the app's <!--size:app.bytes-->5 881 B<!--/size:app.bytes-->, because a bundler
...
VeraJS + own renderer is <!--size:app.bytes-->5 881 B<!--/size:app.bytes--> and requires nothing. That is the price of needing no
...
(<!--size:app.bytes-->5 881 B<!--/size:app.bytes-->) is within ten bytes of Lit
...
pairing (<!--size:app.verajs-lit-html.bytes-->5 353 B<!--/size:app.verajs-lit-html.bytes-->) is lower still. A gap that
small is not a claim in either direction — "level with Lit" is the honest sentence, and it survives
the next commit either way, which "under Lit" does not.
