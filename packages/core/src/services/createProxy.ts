 ...
 * `size` read; notified by `@verajs/store/collections`, which declares the same literal rather than
...
const warnNoCollections = (obj: object) => {
  ...
  console.error(
    `[vera] a ${kind} in a store needs @verajs/store/collections — ` +
      `\`import { collections } from '@verajs/store/collections'\` and add it to your \`wire([…])\` call. ` +
...
const createHandler = <T extends object>(
  ...
  return {
    get(obj: T & StoreProxyKeys, prop: Extract<keyof T, string>, receiver) {
       ...
       * `@verajs/store/collections`). Returning without tracking meant `${state.map.size}` in a template
       * never updated.
       */
      ...
      /**
       * Map/Set methods: re-bound and tracked by `@verajs/store/collections`. Native collection methods
