...
| --- | --- | --- |
| `@verajs/motion` / `./core` | <!--size:motion.gzip.bytes-->12 538 B<!--/size:motion.gzip.bytes--> | compiler + writer: parse the attribute, generate the CSS, name it deterministically |
| `./ssr` | <!--size:motion-ssr.gzip.bytes-->12 369 B<!--/size:motion-ssr.gzip.bytes--> | `renderMotion(document)` — mark every in-scope element, emit one sheet per tree |
