...
describe('parseElement — the grammar', () => {
  ...
  it('reads a bare value as the end of the timeline', () => {
    const parsed = parseElement(el('<div data-vm data-vm-opacity="0"></div>'), ctx());
  ...
  it('keeps a lone start keyframe as authored', () => {
    const parsed = parseElement(el('<div data-vm data-vm-opacity="0% 0"></div>'), ctx());
    expect(frames(animationFor(parsed, 'opacity'))).toEqual([['0%', 0]]);
  });

  it('reads a two-keyframe list', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-translate-y="0% 40px, 100% 0px"></div>'), ctx());
  ...
  it('reads keyframes with no cap on how many', () => {
    const parsed = parseElement(el(`<div data-vm
      data-vm-translate-y="0% 0, 20% 80, 40% 20, 60% 90, 80% 10, 100% 100"></div>`), ctx());
  ...
  it('leaves a percentage-only animation geometry-free', () => {
    const parsed = parseElement(el('<div data-vm data-vm-opacity="0% 0, 100% 1"></div>'), ctx());
  ...
  it('derives the category without it appearing in the attribute', () => {
    const parsed = parseElement(el(`<div data-vm
      data-vm-translate-y="10px" data-vm-blur="4px" data-vm-radius-top-left="8px"></div>`), ctx());
  ...
  it('takes the unit from the value, not a separate attribute', () => {
    const parsed = parseElement(el('<div data-vm data-vm-translate-y="1.5rem"></div>'), ctx());
    expect(animationFor(parsed, 'translate-y').unit).toBe('rem');
  });

  it('falls back to the property default unit', () => {
    const parsed = parseElement(el('<div data-vm data-vm-rotate="90"></div>'), ctx());
    expect(animationFor(parsed, 'rotate').unit).toBe('deg');
  });

  it('drops one bad keyframe rather than the whole property', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0% 0, garbage, 100% 1"></div>'), ctx());
  ...
  it('returns null when nothing valid is found', () => {
    expect(parseElement(el('<div data-vm></div>'), ctx())).toBeNull();
    expect(parseElement(el('<div data-vm data-vm-nonsense="5"></div>'), ctx())).toBeNull();
    expect(parseElement(el('<div data-vm data-vm-opacity="nope"></div>'), ctx())).toBeNull();
  });
});
...
describe('parseElement — width bands', () => {
  ...
  it('reads an inline range', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-translate-y="0% 0px, 100% 100px; [0-500]: 100% 20px"></div>'), ctx());
    const a = animationFor(parsed, 'translate-y');
  ...
  it('reads an open-ended range', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0% 0, 100% 1; [900+]: 100% 0.5"></div>'), ctx());
  ...
  it('refuses junk between the range and its colon rather than shrugging', () => {
    ...
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0% 0, 100% 1; [0-500]x: 100% 0.5"></div>'), ctx());
    const a = animationFor(parsed, 'opacity');
  ...
  it('reads several bands from one attribute', () => {
    const parsed = parseElement(el(`<div data-vm
      data-vm-opacity="0% 0, 100% 1; [0-500]: 100% 0.5; [501-900]: 100% 0.8"></div>`), ctx());
  ...
  it('resolves a registered name to the range it stands for', () => {
    const parsed = parseElement(el(`<div data-vm
      data-vm-translate-y="100px" data-vm-translate-y-mobile="20px"></div>`), ctx(named));
    const a = animationFor(parsed, 'translate-y');
    expect(a.bands).toHaveLength(1);
  ...
  it('ignores a name the instance never registered', () => {
    const parsed = parseElement(el(`<div data-vm
      data-vm-translate-y="100px" data-vm-translate-y-nonsense="20px"></div>`), ctx(named));
    expect(animationFor(parsed, 'translate-y').bands).toHaveLength(0);
  });

  it('rejects a malformed range rather than guessing', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0% 0, 100% 1; [500-100]: 100% 0"></div>'), ctx());
  ...
  it('marks the animation geometry-dependent if any band position is', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0% 0, 100% 1; [0-500]: 50vh 0.5"></div>'), ctx());
...
describe('parseElement — stagger', () => {
  const grid = (attrs, count = 3) => {
    document.body.innerHTML =
      `<div ${attrs}>${'<div data-vm data-vm-opacity="0% 0, 100% 1"></div>'.repeat(count)}</div>`;
    return [...document.querySelectorAll('[data-vm]')].map((n) => parseElement(n, ctx()));
  };

  it('offsets each child by its index, leaving the first alone', () => {
    const [a, b, c] = grid('data-vm-stagger="8"');
  ...
  it('defaults to percent, the unit keyframe positions use most', () => {
    expect(grid('data-vm-stagger="8"')[1].stagger.positionUnit).toBe('%');
  });

  it.each(['%', 'vh', 'vw', 'px', 'rem'])('accepts an explicit %s', (unit) => {
    const [, second] = grid(`data-vm-stagger="10${unit}"`);
  ...
  it('does not stagger the staggering element itself, and does not count it', () => {
    document.body.innerHTML = `<div data-vm data-vm-stagger="8" data-vm-opacity="0% 0, 100% 1">
      <div data-vm data-vm-opacity="0% 0, 100% 1"></div>
      <div data-vm data-vm-opacity="0% 0, 100% 1"></div></div>`;
    const [host, first, second] =
      [...document.querySelectorAll('[data-vm]')].map((n) => parseElement(n, ctx()));
  ...
  it('accepts a negative step, so a row can arrive in reverse', () => {
    expect(grid('data-vm-stagger="-8"')[2].stagger).toEqual({ position: -16, positionUnit: '%' });
  });

  it('reports a malformed step rather than guessing at it', () => {
    const [, second] = grid('data-vm-stagger="soon"');
    expect(second.rejected.join(' | ')).toContain('data-vm-stagger');
    expect(second.stagger).toBeUndefined();
  });

  it('range-checks the step like any other position', () => {
    expect(grid('data-vm-stagger="999999"')[1].stagger).toBeUndefined();
  });

  it('applies through a nesting level, not just to direct children', () => {
    document.body.innerHTML = `<div data-vm-stagger="5">
      <div class="wrap"><div data-vm data-vm-opacity="0% 0, 100% 1"></div></div>
      <div class="wrap"><div data-vm data-vm-opacity="0% 0, 100% 1"></div></div></div>`;
    const parsed = [...document.querySelectorAll('[data-vm]')].map((n) => parseElement(n, ctx()));
...
describe('parseElement — presets', () => {
  it('expands a preset named on the marker', () => {
    const parsed = parseElement(el('<div data-vm="fade-up"></div>'), ctx());
    expect(frames(animationFor(parsed, 'opacity'))).toEqual([['0%', 0], ['100%', 1]]);
  ...
  it('lets an explicit attribute replace the preset for that property', () => {
    const parsed = parseElement(
      el('<div data-vm="fade-up" data-vm-translate-y="0% 200px, 100% 0px"></div>'), ctx());
  ...
  it('records an unknown preset as rejected rather than guessing', () => {
    const parsed = parseElement(el('<div data-vm="not-a-preset" data-vm-opacity="0"></div>'), ctx());
    /** A sentence, not a bare name — same contract as every other refusal (2026-09-01). */
    expect(parsed.rejected.join(' ')).toContain('data-vm="not-a-preset"');
...
describe('parseElement — pinning', () => {
  it('reads a pin length with an explicit unit', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0" data-vm-pin="120px"></div>'), ctx());
    expect(parsed.settings.pin).toBe('120px');
  });

  it('defaults a bare number to px', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0" data-vm-pin="80"></div>'), ctx());
    expect(parsed.settings.pin).toBe('80px');
  });

  it('accepts other length units', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0" data-vm-pin="10vh"></div>'), ctx());
    expect(parsed.settings.pin).toBe('10vh');
  });

  it.each(['calc(10px + 2px)', 'auto', 'url(x)', '10 20', ''])('rejects %j', (bad) => {
    const parsed = parseElement(
      el(`<div data-vm data-vm-opacity="0" data-vm-pin="${bad}"></div>`), ctx());
...
describe('parseElement — settings', () => {
  it('reads a numeric setting', () => {
    const parsed = parseElement(el('<div data-vm data-vm-opacity="0" data-vm-inertia="2"></div>'), ctx());
    expect(parsed.settings.inertia).toBe(2);
  });

  it('reads a bare boolean attribute as true', () => {
    const parsed = parseElement(el('<div data-vm data-vm-opacity="0" data-vm-run-once></div>'), ctx());
    expect(parsed.settings['run-once']).toBe(true);
  });

  it('reads an explicit boolean', () => {
    const parsed = parseElement(el('<div data-vm data-vm-opacity="0" data-vm-run-once="false"></div>'), ctx());
    expect(parsed.settings['run-once']).toBe(false);
  });

  it('rejects a non-numeric value for a numeric setting', () => {
    const parsed = parseElement(el('<div data-vm data-vm-opacity="0" data-vm-inertia="fast"></div>'), ctx());
    expect(parsed.settings.inertia).toBeUndefined();
    expect(parsed.rejected.join(' | ')).toContain('data-vm-inertia');
  });
});

describe('parseElement — settings are range-checked like values', () => {
  const settingsOf = (attrs) =>
    parseElement(el(`<div data-vm data-vm-opacity="0" ${attrs}></div>`), ctx());
   ...
   * rendering `rejected` showed `data-vm-when` and nothing else, while
   ...
   */
  it.each([
    ['inertia above the ceiling', 'data-vm-inertia="99999999"',
      'data-vm-inertia', 'must be a number from 0 to'],
    ['negative inertia', 'data-vm-inertia="-5"',
      'data-vm-inertia', 'must be a number from 0 to'],
    ['frames above the ceiling', 'data-vm-frame-count="1000000000"',
      'data-vm-frame-count', 'must be a number from'],
    ['a frame-pad that would allocate megabytes', 'data-vm-frame-pad="10000000"',
      'data-vm-frame-pad', 'must be a number from'],
    ['an injected second transition', 'data-vm-ease="linear, all 9999s linear"',
      'data-vm-ease', 'is not an easing name or a cubic-bezier()'],
    ['a transform-origin that is not one', 'data-vm-transform-origin="red; position: fixed"',
      'data-vm-transform-origin', 'is not a transform-origin'],
  ...
  it.each([
    ['speed', 'data-vm-inertia="0.4"', { inertia: 0.4 }],
    ['inertia of 0, which means track exactly', 'data-vm-inertia="0"', { inertia: 0 }],
    ['ease', 'data-vm-ease="ease-in-out"', { ease: 'ease-in-out' }],
    ['a bezier ease', 'data-vm-ease="cubic-bezier(0.33, 1, 0.68, 1)"', { ease: 'cubic-bezier(0.33, 1, 0.68, 1)' }],
    ['transform-origin', 'data-vm-transform-origin="50% 50%"', { 'transform-origin': '50% 50%' }],
    ['frames', 'data-vm-frame-count="240"', { 'frame-count': 240 }],
...
describe('parseElement — security', () => {
  it('parses an image-sequence animation', () => {
    const parsed = parseElement(el(`<canvas data-vm
      data-vm-frame="0% 0, 100% 240"
      data-vm-frame-url="/seq/" data-vm-frame-count="240"></canvas>`), ctx());
  ...
  it('accepts path now that offset-path drives it', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-path="0% 0, 100% 100" data-vm-path-selector="#route"></div>'),
      ctx());
  ...
  it.each(['calc(100px + 1px)', 'url(//evil.example)', 'expression(1)', 'var(--x)', '100px;color:red'])(
    'drops the hostile value %j and leaves the element unanimated', (bad) => {
      const parsed = parseElement(el(`<div data-vm data-vm-translate-y="${bad}"></div>`), ctx());
      expect(parsed).toBeNull();
    });

  it('drops only the bad value when others are valid', () => {
    const parsed = parseElement(
      el('<div data-vm data-vm-opacity="0" data-vm-translate-y="calc(1px)"></div>'), ctx());
    expect(animationFor(parsed, 'opacity')).toBeDefined();
    expect(animationFor(parsed, 'translate-y')).toBeUndefined();
  });

  it('ignores foreign data attributes entirely', () => {
    const parsed = parseElement(el(`<div data-vm data-vm-opacity="0"
