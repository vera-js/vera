 ...
 * `data-vd-motion` is the attribute and `vd:motion:*` the events — the `vd:`
 * family prefix is the same one `persist` uses for its storage keys, so
 * everything this system writes into a page's shared namespaces is findable
 * with one grep.
 */
const NAMESPACE = 'vd:motion';
export const EVENTS = {
